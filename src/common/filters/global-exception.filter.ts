...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      String(exception.message).toLowerCase().includes("fetch")
    ) {
      ...
      /(ENOTFOUND|ECONNREFUSED|ECONNRESET|EAI_AGAIN)/.test(
        String((exception as any).code),
      )
    ) {
      status = HttpStatus.BAD_GATEWAY;
    ...
    console.error(
      "[GlobalExceptionFilter] Sending error response:",
      errorResponse,
    );
