...
export class InternsService {
  ...
  async create(createInternDto: CreateInternDto): Promise<Intern> {
    const { data: intern, error } = await this.supabaseService
      .getClient()
      .from("interns")
  ...
  async findAll(filterDto: FilterInternsDto) {
    ...
    let query = this.supabaseService.getClient().from("interns").select(`
        *,
        users!inner(*),
        divisions!inner(*)
      `);
  ...
  async findByAngkatan(angkatan: number) {
    const { data: interns } = await this.supabaseService
      .getClient()
      .from("interns")
      .select(
        `
