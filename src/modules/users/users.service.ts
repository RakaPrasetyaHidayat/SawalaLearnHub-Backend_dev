...
export class UsersService {
  ...
  private readonly USERS_SELECT_PUBLIC =
    "id,email,full_name,role,status,channel_year,angkatan,created_at,updated_at,school_name,division_id";
  private readonly USERS_SELECT_INTERNAL =
    "id,email,full_name,password,role,status,channel_year,angkatan,created_at,updated_at,school_name,division_id";
  ...
  async getUsersByDivision(division_id: string, year?: string) {
    try {
      ...
      const tryView = async () => {
        try {
          let q = client
            ...
            .select(
              "id, full_name, role, status, division_id, division_name, school_name, channel_year",
            );
          ...
          if (isUuid) q = q.eq("division_id", division_id);
            ...
            q = q.or(
              `division_name.eq.${normalized},division_name.ilike.%${division_id}%`,
            );
          }
      ...
      if (viewResult !== null) {
        ...
        const result = Object.entries(counts).map(([channel_year, count]) => ({
          channel_year,
          count,
        }));
      ...
      const usersQ = client
        .from("users")
        .select(
          "id, full_name, role, status, division_id, school_name, channel_year",
        );
      ...
      if (!isUuid) {
        ...
        const matched = (divisions || []).filter(
          (d: any) =>
            String(d.name || "").toUpperCase() === matchNorm ||
            String(d.name || "")
              .toLowerCase()
              .includes(String(division_id).toLowerCase()),
        );
      ...
      if (!isNaN(Number(yNum))) {
        users = users.filter(
          (u: any) => Number(u.channel_year) === Number(yNum),
        );
        return {
          status: "success",
          message: "Users retrieved successfully for the specified year",
  ...
  async getDivisionIdsByYear(year: string, onlyApproved = true) {
    try {
      ...
      const result = Object.entries(counts).map(([division_id, count]) => ({
        division_id,
        count,
      }));
      ...
      try {
        const { data: divisions } = await this.supabaseService
          .getClient(true)
          .from("divisions")
          .select("id,name");
        ...
        const enriched = result.map((r) => ({
          ...
          division_name: byId[r.division_id] || null,
          count: r.count,
        }));
        return {
          status: "success",
    ...
    try {
      ...
      if (error.message?.includes("timeout")) {
        return {
          ...
          message:
            "Database is taking too long to respond. Check SUPABASE_URL config on Vercel.",
          fallback: true,
  ...
  async upsertUserStatus(
    userId: string,
    dto: { status?: UserStatus; role?: UserRole },
    adminId: string,
  ) {
    ...
    if (![UserRole.ADMIN, UserRole.SISWA].includes(roleToSet)) {
      throw new BadRequestException(
        "Invalid role. Allowed values: ADMIN, SISWA",
      );
    }
  ...
  async updateUserStatus(
    userId: string,
    updateStatusDto: { status: UserStatus; role?: UserRole },
    adminId: string,
  ) {
  ...
  async acceptUser(userId: string, adminId: string, role?: UserRole) {
    return this.upsertUserStatus(
      userId,
      { status: UserStatus.APPROVED, role },
      adminId,
    );
  }
  ...
  async updateOwnProfile(userId: string, update: any) {
    try {
      ...
      if (typeof update.full_name !== "undefined")
        payload.full_name = update.full_name;
      else if (typeof update.name !== "undefined")
        payload.full_name = update.name;

      // Support school OR school_name
      if (typeof update.school_name !== "undefined")
        payload.school_name = update.school_name;
      else if (typeof update.school !== "undefined")
        payload.school_name = update.school;
      ...
      if (typeof update.division !== "undefined" && update.division !== null) {
        ...
        const { data: existingUserRow, error: userRowErr } = await client
          .from("users")
          .select("*")
          .eq("id", userId)
          .maybeSingle();
        ...
        if (matchedDivisions && matchedDivisions.length === 1) {
          ...
          if (
            existingUserRow &&
            Object.prototype.hasOwnProperty.call(existingUserRow, "division_id")
          ) {
          ...
          const exactCi = matchedDivisions.find(
            (d: any) => String(d.name).toLowerCase() === raw.toLowerCase(),
          );
          if (exactCi) {
            if (
              existingUserRow &&
            ...
            // ambiguous matches
            throw new Error(
              "Multiple divisions matched; please pick the exact division from the dropdown",
            );
          }
        } else {
          ...
          if (normalized.includes("backend")) canonical = "BACKEND";
            ...
            canonical = "FRONTEND";
          else if (normalized.includes("ui") || normalized.includes("ux"))
            canonical = "UI_UX";
          ...
          if (
            ...
            if (byName && byName.length === 1)
              payload.division_id = byName[0].id;
            else {
      ...
      if (typeof update.channel_year !== "undefined")
        payload.channel_year = update.channel_year;
  ...
  async updateUserProfile(userId: string, update: any, adminId: string) {
    ...
    if (typeof update.school_name !== "undefined")
      payload.school_name = update.school_name;
    if (typeof update.division_id !== "undefined")
    ...
    if (
      payload.division_id &&
      typeof payload.division_id === "string" &&
      ...
      if (divisions.length > 1) {
        throw new BadRequestException(
          "Multiple divisions matched; provide a division UUID",
        );
      }
