...
export class UsersController {
      ...
      data: {
        ...
        endpoints: {
          ...
          acceptUser: {
            ...
            description:
              "Accept user (set APPROVED and optional role, default SISWA)",
            auth: "Required (Admin)",
          },
          deleteUser: {
            method: "DELETE",
  ...
  @Get("all")
  @UseGuards(JwtAuthGuard)
  async getAllUsers(@GetUser("role") role: UserRole) {
    ...
    const filtered =
      role === UserRole.ADMIN
        ? users
        : Array.isArray(users)
          ? users.filter((u: any) => u.status === UserStatus.APPROVED)
          : [];
  ...
  @Roles(UserRole.ADMIN)
  @UseGuards(JwtAuthGuard, RolesGuard)
  deleteRejectedUser(@Param("id") id: string, @GetUser("id") adminId: string) {
    return this.usersService.deleteRejectedUser(id, adminId);
  }

  @Get("pending")
  @UseGuards(JwtAuthGuard)
  async getPendingUsers(@GetUser("role") role: UserRole) {
  ...
  async uploadAvatar(
    @GetUser("id") userId: string,
    @UploadedFiles()
    files?: { file?: Express.Multer.File[]; avatar?: Express.Multer.File[] },
  ) {
    try {
      const file =
        (files?.file && files.file[0]) || (files?.avatar && files.avatar[0]);
      if (!file || !file.buffer) {
        throw new HttpException(
  ...
  async updateUserProfileByAdmin(
    ...
    try {
      const updated = await this.usersService.updateUserProfile(
        id,
        updateDto,
        adminId,
      );
