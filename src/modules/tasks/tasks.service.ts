...
export class TasksService extends BaseService {
  ...
  async createTask(
    ...
    try {
      console.debug("[TasksService.createTask] payload:", {
        createTaskDto,
        adminId,
        hasFile: !!file,
      });
      ...
      if (file && file.buffer) {
        ...
        if (uploadErr) {
          ...
          const { data: uploadData2, error: uploadErr2 } =
            await adminClient.storage
              .from("task-files")
              .upload(filePath, file.buffer, { contentType: file.mimetype });
          if (uploadErr2) throw uploadErr2;
          fileUrl = adminClient.storage
            .from("task-files")
            .getPublicUrl(uploadData2.path).data.publicUrl;
        } else {
      ...
      const rawDivisionInput = String(
        (createTaskDto as any).division || createTaskDto.division_id || "",
      ).trim();

      // Fetch all divisions and attempt a local tolerant match first
      const { data: allDivisions, error: allDivErr } =
      ...
      if (allDivisions && allDivisions.length > 0) {
        const exactMatches = allDivisions.filter(
          (d) => normalize(d.name) === inputNorm,
        );
        if (exactMatches.length === 1) {
          ...
          const ciMatches = allDivisions.filter(
            (d) =>
              (d.name || "").toLowerCase() === rawDivisionInput.toLowerCase(),
          );
      ...
      if (!resolvedDivisionId) {
        ...
        if (!divisions2 || divisions2.length === 0) {
          throw new BadRequestException(
            `Invalid division name. Must be one of: BACKEND, FRONTEND, UI_UX, DEVOPS`,
          );
        }
      ...
      const taskPayload: any = {
        ...
        description:
          createTaskDto.description + (fileUrl ? `\nFile: ${fileUrl}` : ""),
        deadline: createTaskDto.deadline,
        // prefer channel_year property; fall back to angkatan for older clients
        channel_year:
          typeof (createTaskDto as any).channel_year !== "undefined"
            ? Number((createTaskDto as any).channel_year)
            : createTaskDto.angkatan
              ? Number(createTaskDto.angkatan)
              : null,
        created_by: adminId,
  ...
  async submitTask(
    taskId: string,
    userId: string,
    submitTaskDto: Partial<SubmitTaskDto> = {},
    file?: Express.Multer.File,
  ) {
    try {
      console.log("[TasksService.submitTask] Starting submission:", {
        taskId,
        userId,
        submitTaskDto,
        hasFile: !!file,
      ...
      console.log(
        "[TasksService.submitTask] Resolved task ID:",
        resolvedTaskId,
      );
      ...
      if (existingErr && existingErr.code !== "PGRST116") {
        ...
        console.error(
          "[TasksService.submitTask] Error checking existing submission:",
          existingErr,
        );
        throw existingErr;
      }
      ...
      if (existingSubmission) {
        console.log(
          "[TasksService.submitTask] Found existing submission:",
          existingSubmission,
        );
        throw new BadRequestException(
      ...
      console.log(
        "[TasksService.submitTask] No existing submission found, proceeding with new submission",
      );
      ...
      if (file && file.buffer) {
        ...
        if (uploadErr) {
          console.error(
            "[TasksService.submitTask] File upload error:",
            uploadErr,
          );
          throw uploadErr;
        }
      ...
      if (isOverdue) {
        ...
        console.log(
          "[TasksService.submitTask] Inserting OVERDUE payload:",
          payload,
        );
        const { data: inserted, error: insertErr } = await adminClient
          ...
          .insert(payload)
          .select(
            "id, task_id, user_id, description, submission_status, file_urls, created_at, updated_at",
          )
          .single();
        if (insertErr) {
          console.error(
            "[TasksService.submitTask] Insert error (OVERDUE):",
            insertErr,
          );
          throw insertErr;
        }
        console.log(
          "[TasksService.submitTask] Successfully inserted (OVERDUE):",
          inserted,
        );
      ...
      console.log(
        "[TasksService.submitTask] Inserting SUBMITTED payload:",
        payload,
      );
      const { data: inserted, error: insertErr } = await adminClient
        .from("task_submissions")
      ...
      if (insertErr) {
        console.error(
          "[TasksService.submitTask] Insert error (SUBMITTED):",
          insertErr,
        );
        throw insertErr;
      }
      console.log(
        "[TasksService.submitTask] Successfully inserted (SUBMITTED):",
        inserted,
      );
   ...
   */
  async submitTaskFlexible(
    userId: string,
    dto: Partial<SubmitTaskDto> = {},
    file?: Express.Multer.File,
  ) {
    try {
      const taskIdentifier = (
        (dto && (dto.task_id || dto.task_identifier)) ||
        ""
      )
        .toString()
        .trim();
      ...
      if (!user || !user.division_id) {
        throw new BadRequestException(
          "User division is not set; cannot auto-select task",
        );
      }
      ...
      if (!uuidRegex.test(String(resolvedDivisionForQuery))) {
        ...
        if (!divErr && divsExact && divsExact.length === 1) {
          ...
          if (!divErr2 && divsF && divsF.length === 1)
            resolvedDivisionForQuery = divsF[0].id;
        }
      }
      ...
      let pick =
        candidates.find((t: any) => !t.deadline || t.deadline >= nowIso) ||
        candidates[0];
      ...
      if (!pick) {
        ...
        pick =
          wide.find((t: any) => !t.deadline || t.deadline >= nowIso) || wide[0];
      }

      if (!pick) {
        throw new NotFoundException(
          "No available tasks to submit for your division/year",
        );
      }
  ...
  async updateTaskStatusFlexible(
    ...
    if (!uuidRegex.test(taskId)) {
      ...
      if (tasksExact && tasksExact.length === 1) {
        ...
        throw new BadRequestException(
          "Multiple tasks matched the provided task identifier; please provide a task UUID",
        );
      } else {
    ...
    if (!uuidRegex.test(userId)) {
      ...
      if (usersByEmail && usersByEmail.length === 1) {
        ...
        if (usersByName && usersByName.length === 1) {
          ...
          (usersByName && usersByName.length > 1) ||
          (usersByEmail && usersByEmail.length > 1)
        ) {
          throw new BadRequestException(
            "Multiple users matched the provided user identifier; please provide a user UUID or unique email",
          );
        } else {
  ...
  async submitTaskDirect(
    ...
    if (existing)
      throw new BadRequestException("Task already submitted by this user");
  ...
  async getUserSubmission(taskId: string, userId: string) {
    try {
      console.log("[TasksService.getUserSubmission] fetch for", {
        taskId,
        userId,
      });
      const adminClient = this.supabaseService.getClient(true);
      ...
      if (error) {
        ...
        if (error.code === "PGRST116") {
          // no submission found
          throw new NotFoundException(
            `No submission found for task '${taskId}' and user '${userId}'`,
          );
        }
        throw error;
      }
      console.log(
        "[TasksService.getUserSubmission] found submission:",
        submission,
      );
    ...
    try {
      ...
      const formattedTasks = (tasks as TaskWithDivision[]).map((task) => {
        ...
        if (Array.isArray(task.division)) {
          divisionName = task.division.length
            ? (task.division[0] as Division).name
            : null;
  ...
  async getTaskSubmissionById(submissionId: string) {
    try {
      console.log(
        "[TasksService.getTaskSubmissionById] Getting submission:",
        submissionId,
      );
      ...
      if (error) {
        ...
        if (error.code === "PGRST116") {
          throw new NotFoundException(
            `Task submission with ID '${submissionId}' not found`,
          );
        }
        throw error;
      }
      ...
      console.log(
        "[TasksService.getTaskSubmissionById] Found submission:",
        submission,
      );
      return {
        status: "success",
      ...
      console.error(
        "[TasksService.getTaskSubmissionById] Error occurred:",
        error,
      );
      if (error instanceof NotFoundException) {
        throw error;
      ...
      throw new BadRequestException(
        error.message || "Failed to get task submission",
      );
    }
  }
  ...
  async getAllSubmissionsForTask(taskId: string) {
    try {
      ...
      throw new BadRequestException(
        error.message || "Failed to list submissions",
      );
    }
  }

  // Admin: update single submission status (taskId + userId)
  async updateTaskSubmissionStatus(
    taskId: string,
    userId: string,
    adminId: string,
    updateTaskDto: UpdateTaskDto,
    ...
    try {
      ...
      const { data: admin, error: adminErr } = await adminClient
        ...
        .eq("id", adminId)
        .single();
      if (adminErr || !admin || admin.role !== UserRole.ADMIN)
  ...
  async batchPreview(filters: any) {
    ...
    let query = adminClient
      .from("task_submissions")
      .select("task_id,user_id", { count: "exact" });

    // Apply filters
    if (filters?.task_ids && filters.task_ids.length)
      query = query.in("task_id", filters.task_ids);
    ...
    if (filters?.due_date_before)
      query = query.lte("created_at", filters.due_date_before);
    if (filters?.assigned_after)
      query = query.gte("created_at", filters.assigned_after);
  ...
  async batchUpdateStatus(
    ...
    for (let i = 0; i < updates.length; i += chunkSize) {
      ...
      // Build filters
      const ors = chunk
        .map((c, idx) => `task_id.eq.${c.task_id},user_id.eq.${c.user_id}`)
        .join(",");
  ...
  async findTaskById(taskId: string) {
    try {
      ...
      const normalizedSubmissions = (submissions || []).map(
        (submission: any) => {
  ...
  async getAllTasks(query: any, accessToken?: string) {
      ...
      if (uuidRegex.test(maybe)) {
        ...
        if (divisions && divisions.length === 1) {
          ...
          if (divisions2 && divisions2.length === 1)
            resolvedQueryDivisionId = divisions2[0].id;
    ...
    try {
      ...
      if (msg.includes('role "') && msg.includes("does not exist")) {
        const adminClient = this.supabaseService.getClient(true);
        let adminBuilder = adminClient
          .from("tasks")
          .select("*", { count: "exact" })
          .order("created_at", { ascending: false });
        if (query?.division_id)
          adminBuilder = adminBuilder.eq(
            "division_id",
        ...
        const {
          ...
          count,
          error: adminErr,
        } = await adminBuilder.range(from, to);
        if (adminErr) throw adminErr;
